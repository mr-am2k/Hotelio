...
void prikazi_rezervacije(){
	...
	if(ulaz.is_open()){
		while(ulaz >> r->kod >> r->broj_sobe >> r->username >> r->datumPrijave.dan >> r->datumPrijave.mjesec >> r->datumPrijave.godina
			...
			cout<<setw(5)<<r->kod<<setw(10)<<r->broj_sobe<<setw(18)<<r->username<<setw(16)<<datumPrijave<<setw(14)<<datumOdjave<<setw(14)<<temp<<endl;
		}
		ulaz.close();
		cout<<"-------------------------------------------------------------------------------------\n";
		delete r;	
...
void stanje_hotela(){
	...
	int pozicija;
	string temp;
	cout<<setw(5)<<"ID"<<setw(10)<<"Ime"<<setw(10)<<"Prezime"<<setw(15)<<"Pozicija"<<setw(10)<<"Plata"<<endl;
	poruka(1);
	...
	if(ulaz3.is_open()){
		while(ulaz3 >> r->id >> r->ime >> r->prezime >> pozicija >> r->plata){
			...
			cout<<setw(5)<<r->id<<setw(10)<<r->ime<<setw(10)<<r->prezime<<setw(15)<<temp<<setw(10)<<r->plata<<endl; 
		}
		ulaz3.close();
		delete r;
		poruka(1);
